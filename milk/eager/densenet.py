...
from tensorflow.keras.layers import (Conv2D, Dropout, BatchNormalization, 
  Dense, AveragePooling2D, MaxPooling2D, GlobalAveragePooling2D)
...
class ConvBlock(tf.keras.Model):
  ...
  def __init__(self, num_filters, data_format, bottleneck, weight_decay=1e-4,
    ...
    self.batchnorm1 = BatchNormalization(trainable=True, momentum=0.9, axis=axis)
    ...
    if self.bottleneck:
      ...
      self.batchnorm2 = BatchNormalization(trainable=True, momentum=0.9, axis=axis)
...
class TransitionBlock(tf.keras.Model):
  ...
  def __init__(self, num_filters, data_format,
    ...
    self.batchnorm = BatchNormalization(trainable=True, momentum=0.9, axis=axis)
...
class DenseNetEager(tf.keras.Model):
  ...
  def call(self, x, training=True):
    ...
    if self.pool_initial:
      ...
      # if verbose:
      #   print('DeseNet Pool Initial:')
      #   print('\toutput: ', output.get_shape())

    # if verbose:
    #   print('DenseNet dense blocks:')
    for i in range(self.num_of_blocks - 1):
      ...
      # print('DenseNet block {}:'.format(i), output.get_shape())
      # if verbose:
      #   print('\t{:02d} output:'.format(i), output.get_shape())
    ...
    # if verbose:
    #   print('DenseNet final dense block:')
    #   print('\toutput: ', output.get_shape())
