...
def draw_projection_with_images(z, attentions, 
  ...
  plt.scatter(z[:-1,0], z[:-1,1], c=attentions, s=10.0, 
    cmap='hot_r',
    # alpha=0.5,
    linewidths=0.5,
    edgecolors='k')
...
"""
def draw_projection(features, features_avg, attentions, savepath=None):
  plt.figure(figsize=(3,3), dpi=300)

  features = np.concatenate([features, features_avg], axis=0)

  z = clusterfn(features)

  plt.scatter(z[:-1,0], z[:-1,1], c=attentions, s=15.0, 
    cmap='hot_r',
    linewidths=0.5,
    edgecolors='k')
  plt.colorbar()
...
def main(args):
  ...
  model = MilkEager( encoder_args=encoder_args , mil_type=args.mil, deep_classifier=True)
  ...
  for test_case in test_list:
    ...
    savepath = '{}_{}_{:3.2f}_yhat.png'.format(args.savebase, case_name, yhat[0,1])
    print('Saving figure {}'.format(savepath))
    z = draw_projection(features, features_att, yhat_1, savepath=savepath)

    savepath = '{}_{}_{:3.2f}_yhat_img.png'.format(args.savebase, case_name, yhat[0,1])
    print('Saving figure {}'.format(savepath))
    draw_projection_with_images(z, yhat_1, 
...
if __name__ == '__main__':
  ...
  parser.add_argument('--mil', default='average', type=str)
